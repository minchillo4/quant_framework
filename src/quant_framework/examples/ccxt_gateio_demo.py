"""
Example demonstrating CCXT adapter usage with Gate.io.

This script shows how to:
1. Create Gate.io adapters
2. Build Instrument models
3. Fetch OHLCV and Open Interest data
4. Use preprocessing pipeline

Run with:
    python -m quant_framework.examples.ccxt_gateio_demo
"""
...
from quant_framework.ingestion.adapters.ccxt_plugin.builders import (
    create_gateio_adapters,
)
from quant_framework.ingestion.preprocessing.providers import GateIOPreprocessor
...
async def demo_gateio_linear_ohlcv():
    """Fetch OHLCV data from Gate.io linear perpetuals using CCXT adapter."""
    logger.info("=" * 60)
    logger.info("DEMO: Gate.io Linear Perpetual OHLCV via CCXT")
    logger.info("=" * 60)

    # Create adapters
    ohlcv_adapter, oi_adapter = create_gateio_adapters(
    ...
    instrument = Instrument(
        ...
        base_asset="BTC",
        quote_asset="USDT",
        raw_symbol="BTC/USDT:USDT",  # Gate.io format
        is_inverse=False,
        is_active=True,
    )
...
async def demo_gateio_open_interest():
    """Fetch Open Interest data from Gate.io using CCXT adapter."""
    logger.info("=" * 60)
    logger.info("DEMO: Gate.io Open Interest via CCXT")
    logger.info("=" * 60)
...
async def demo_gateio_spot():
    """Fetch OHLCV data from Gate.io spot using CCXT adapter."""
    logger.info("=" * 60)
    logger.info("DEMO: Gate.io Spot OHLCV via CCXT")
    logger.info("=" * 60)

    # Create adapters
    ohlcv_adapter, _ = create_gateio_adapters(
    ...
    instrument = Instrument(
        ...
        base_asset="BTC",
        quote_asset="USDT",
        raw_symbol="BTC/USDT",  # Gate.io spot format
        is_inverse=False,
        is_active=True,
    )
...
async def main():
    """Run all Gate.io demos."""
    logger.info("Starting Gate.io CCXT adapter demos")

    await demo_gateio_linear_ohlcv()
    await demo_gateio_open_interest()
    await demo_gateio_spot()

    logger.info("All Gate.io demos complete!")
