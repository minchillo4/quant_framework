...
This module implements DATABASE-BACKED checkpoints for backfill orchestration.
This is separate from MinIO-backed checkpoints in infrastructure.checkpoint module:

1. **Database-Backed Checkpoints (THIS MODULE)**
   ...
   - Metadata: started_at, updated_at, chunks_completed, records_written
   - Purpose: Resumable backfill orchestration with detailed progress

2. **MinIO-Backed Checkpoints (infrastructure.checkpoint)**
   - Location: MinIO S3-compatible storage (JSON documents)
   - Used by: CCXT incremental pipelines (1h, 5m data)
   ...
   - Purpose: Atomic checkpoint updates for streaming pipelines

Use the DATABASE approach for batch backfills (this module).
Use the MinIO approach for incremental streaming (infrastructure.checkpoint).
"""
