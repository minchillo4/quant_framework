"""
CCXT 1h Open Interest Incremental DAG - Linear Perpetuals.

Continuously updates Open Interest data every hour by fetching from CCXT exchanges
...
2. This DAG auto-triggers every 5 minutes → Incremental updates to MinIO bronze
3. Checkpoint coordination ensures no gaps or duplicates

Schedule: Every hour (0 * * * *)
Exchanges: binance, bybit, gateio, huobi (linear perpetuals only)
Symbols: BTC, ETH (from config, as BTCUSDT, ETHUSDT)
Market Type: linear_perpetual
...
def load_config() -> dict:
    ...
    config_path = Path("config/services/oi_incremental_5m_linear.yaml")
    try:
...
def build_fetch_combinations(config: dict) -> list[dict]:
    """Build all symbol/exchange/market_type combinations to fetch."""
    combos: list[dict] = []
    exchanges = config["fetching"]["exchanges"]
    symbols = config["fetching"]["symbols"]

    for exchange in exchanges:
        market_type = "linear_perpetual"  # enforce linear for this DAG
        for symbol in symbols:
            # For linear perpetuals: BTC/USDT:USDT, ETH/USDT:USDT
            # Quote and settlement are both USDT
            combos.append(
                {
                    "exchange": exchange,
    ...
    logger.info(f"📋 Built {len(combos)} fetch combinations")
    return combos


@task
def process_combo(combo: dict, config: dict) -> dict:
    """Process a single exchange/symbol combination (linear).
    ...
    async def _run():
        try:
            ...
            # For linear perpetuals: BTC/USDT:USDT
            ...
            try:
                ...
                # Create proper instrument_id: BTCUSDT_PERP_BINANCE_USDM
                instrument_id = (
...
@dag(
    dag_id="ccxt_oi_1h_linear_incremental",
    description="CCXT 1h Open Interest incremental fetcher for linear perpetuals",
    schedule="0 * * * *",
    ...
    tags=["ccxt", "oi", "1h", "linear"],
    doc_md=__doc__,
    is_paused_upon_creation=False,
)
def ccxt_oi_1h_linear_incremental():
    """CCXT 1h OI incremental DAG."""
...
# Instantiate the DAG
ccxt_oi_1h_linear_dag = ccxt_oi_1h_linear_incremental()
